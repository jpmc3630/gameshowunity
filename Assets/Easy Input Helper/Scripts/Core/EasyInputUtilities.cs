...
namespace EasyInput.Core
{
    ...
    public static class EasyInputUtilities
    {
        ...
        public static Vector2 flipCoordinates(EasyInputConstants.REMOTE_ORIENTATION orientation, Vector2 touch)
        {
            Vector2 temp;
            temp = touch;

            if (orientation == EasyInputConstants.REMOTE_ORIENTATION.TouchpadLeft)
            {
                //holding it to the left so x = -y and y = x
                touch.x = -temp.y;
                touch.y = temp.x;
            ...
            {
                //holding it to the right so x = -y and y = x
                touch.x = temp.y;
                touch.y = -temp.x;
